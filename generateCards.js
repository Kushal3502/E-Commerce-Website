export async function generateCards(container, data) {
    ...
    document.querySelectorAll('.add-to-cart-btn').forEach(btn => btn.addEventListener('click', addToLocalStorage))
}
...
function addToLocalStorage(e) {
    if (e.target && e.target.classList.contains('add-to-cart-btn')) {
        ...
        const productName = e.target.parentNode.parentNode.querySelector('.title').textContent;
        const quantity = 1;
        const price = Number(e.target.parentNode.parentNode.querySelector('.price').textContent.split(' ')[1]);
