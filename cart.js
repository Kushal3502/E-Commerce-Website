...
function showProductInCart(data) {
  data.forEach((item) => {
    ...
    priceCardItemPrice.textContent = `Rs. ${(
  ...
  document
    ...
    .forEach((btn) => btn.addEventListener("click", manageQuantity));
...
function manageQuantity(e) {
  ...
  if (quantity !== 0) {
    ...
    item.children[1].textContent = `Rs. ${(
...
function updateTotalPrice() {
  ...
  amount.textContent = `Total cart value is: Rs. ${totalPrice.toFixed(2)}`;
}
